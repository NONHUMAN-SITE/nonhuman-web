...
plt.xscale('log')
plt.yscale('log')
plt.xlabel('$d_k$ (log scale)', fontsize=12)
...
![Decay of the Softmax Jacobian Norm with Large $d_k$|600x600](https://res.cloudinary.com/dtpiuha91/image/upload/v1738512008/cmrssnt8kll82vxhmdar.png)
...
![Decay of the Softmax Jacobian Norm with Large $d_k$|600x600](https://res.cloudinary.com/dtpiuha91/image/upload/v1738512047/ryfcogoazzxrswzkivnw.png)
...
In the next blog, we will explore how we can implement all of the above in code and also go over the process for training a small language model from scratch.

[Implementation Blog](www.nonhuman.site/wiki/MIND/1.2)
